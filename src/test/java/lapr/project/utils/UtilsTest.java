...
import lapr.project.controller.LeaveBikeController;
import lapr.project.controller.RequestBikeController;
...
class UtilsTest {
    ...
    void setUp() {
        ...
        user1.setAvgSpeed(20 * 1000 / 3600.0);
    ...
    @Test
    void calculateFlatDistanceBetweenTwoCoordinatesTest() {
        double lisboaLatitude = 38.7222524;
        double lisboaLongitude = -9.1393366;
        double portoLatitude = 41.1579438;
        double portoLongitude = -8.6291053;
        double expected_OPO_LIS = 274305.96;
        assertTrue(Math.abs(expected_OPO_LIS - Utils.calculateFlatDistanceBetweenTwoCoordinates(portoLatitude, portoLongitude, lisboaLatitude, lisboaLongitude)) < 0.01);
        assertTrue(Math.abs(expected_OPO_LIS - Utils.calculateFlatDistanceBetweenTwoCoordinates(lisboaLatitude, lisboaLongitude, portoLatitude, portoLongitude)) < 0.01);

        double barcelonaLatitude = 41.382160;
        double barcelonaLongitude = 2.172407;
        double expected_OPO_BCN = 902497.41;
        assertTrue(Math.abs(expected_OPO_BCN - Utils.calculateFlatDistanceBetweenTwoCoordinates(portoLatitude, portoLongitude, barcelonaLatitude, barcelonaLongitude)) < 0.01);
        assertTrue(Math.abs(expected_OPO_BCN - Utils.calculateFlatDistanceBetweenTwoCoordinates(barcelonaLatitude, barcelonaLongitude, portoLatitude, portoLongitude)) < 0.01);

        double expected_LIS_BCN = 1006327.20;
        assertTrue(Math.abs(expected_LIS_BCN - Utils.calculateFlatDistanceBetweenTwoCoordinates(lisboaLatitude, lisboaLongitude, barcelonaLatitude, barcelonaLongitude)) < 0.01);
        assertTrue(Math.abs(expected_LIS_BCN - Utils.calculateFlatDistanceBetweenTwoCoordinates(barcelonaLatitude, barcelonaLongitude, lisboaLatitude, lisboaLongitude)) < 0.01);
    }

    @Test
    void calculateDistanceBetweenTwoCoordinatesTest() {
        double height = 50.22;
        double lisboaLatitude = 38.7222524;
        double lisboaLongitude = -9.1393366;
        double portoLatitude = 41.1579438;
        double portoLongitude = -8.6291053;
        double expected_OPO_LIS = 274305.96;
        double expected_OPO_LIS_h = Math.sqrt(Math.pow(expected_OPO_LIS, 2) + Math.pow(height, 2));
        assertTrue(Math.abs(expected_OPO_LIS_h - Utils.calculateDistanceBetweenTwoCoordinates(portoLatitude, portoLongitude, 0.0, lisboaLatitude, lisboaLongitude, height)) < 0.01);
        assertTrue(Math.abs(expected_OPO_LIS_h - Utils.calculateDistanceBetweenTwoCoordinates(lisboaLatitude, lisboaLongitude, 0.0, portoLatitude, portoLongitude, height)) < 0.01);

        double barcelonaLatitude = 41.382160;
        double barcelonaLongitude = 2.172407;
        double expected_OPO_BCN = 902497.41;
        double expected_OPO_BCN_h = Math.sqrt(Math.pow(expected_OPO_BCN, 2) + Math.pow(height, 2));
        assertTrue(Math.abs(expected_OPO_BCN_h - Utils.calculateDistanceBetweenTwoCoordinates(portoLatitude, portoLongitude, 0.0, barcelonaLatitude, barcelonaLongitude, height)) < 0.01);
        assertTrue(Math.abs(expected_OPO_BCN_h - Utils.calculateDistanceBetweenTwoCoordinates(barcelonaLatitude, barcelonaLongitude, 0.0, portoLatitude, portoLongitude, height)) < 0.01);

        double expected_LIS_BCN = 1006327.20;
        double expected_LIS_BCN_h = Math.sqrt(Math.pow(expected_LIS_BCN, 2) + Math.pow(height, 2));
        assertTrue(Math.abs(expected_LIS_BCN_h - Utils.calculateDistanceBetweenTwoCoordinates(lisboaLatitude, lisboaLongitude, 0.0, barcelonaLatitude, barcelonaLongitude, height)) < 0.01);
        assertTrue(Math.abs(expected_LIS_BCN_h - Utils.calculateDistanceBetweenTwoCoordinates(barcelonaLatitude, barcelonaLongitude, 0.0, lisboaLatitude, lisboaLongitude, height)) < 0.01);
    }

    @Test
    void getTimeDifferenceInMinutesTest() {
        long resultWithString = Utils.getTimeDifferenceInMinutes("2018-10-10 16:16:16", "2018-10-10 16:20:16");
        long resultWithDate = Utils.getTimeDifferenceInMinutes(Utils.convertStringToDate("2018-10-10 16:16:16"), Utils.convertStringToDate("2018-10-10 16:20:16"));
        assertEquals(4, resultWithString);
        assertEquals(4, resultWithDate);
    }

    @Test
    void testCalculateEnergyToTravelBetweenLocations() {
        ...
        double expected = 12.30;//119.10
        double val = Utils.calculateEnergyToTravelBetweenLocations(null, null, cqueijo, ffoz, null, route1);
        double diff = Math.abs(expected - val);
        assertTrue(diff < 0.01);

        expected = 0.0;
        Route routeTmp2 = controller.getRouteByCoordinates(cqueijo.getGeoLatitude(), cqueijo.getGeoLongitude(), cqueijo.getGeoLatitude(), cqueijo.getGeoLongitude());
        val = Utils.calculateEnergyToTravelBetweenLocations(null, null, cqueijo, cqueijo, null, routeTmp2);
        ...
        val = Utils.calculateEnergyToTravelBetweenLocations(null, null, cqueijo, ffoz, null, route1);
        diff = Math.abs(expected - val);
        assertTrue(diff < 0.01);
        ...
        expected = 13.66;
        route1.setWindSpeed(0.0);
        val = Utils.calculateEnergyToTravelBetweenLocations(user1, null, cqueijo, ffoz, null, route1);
        diff = Math.abs(expected - val);
        assertTrue(diff < 0.01);
        ...
        // faltam testes com diferentes alturas
        // combinações de alturas e ventos
    }

    @Test
    void convertHoursToHoursMinutesSecondsTest() {
        /*
        ...
        */
        ...
        Double hoursTest = 7.66;
        Double expectedHours = 7.0;
        Double expectedMinutes = 39.0;
        Double expectedSeconds = 36.0;
        String expected = String.format("%s(h) %s(m) %s(s)", expectedHours.intValue(), expectedMinutes.intValue(), expectedSeconds.intValue());
        assertEquals(Utils.convertHoursToHoursMinutesSeconds(hoursTest), expected);

        hoursTest = 0.0;
        expected = String.format("%s(h) %s(m) %s(s)", 0, 0, 0);
        assertEquals(Utils.convertHoursToHoursMinutesSeconds(hoursTest), expected);

        hoursTest = null;
        assertNull(Utils.convertHoursToHoursMinutesSeconds(hoursTest));
    }

    @Test
    void getTimeDifferenceInDaysTest() {
        Long resultWithString = Utils.getTimeDifferenceInDays("2018-10-20 16:16:16", "2018-10-10 16:16:16");
        Long resultWithDate = Utils.getTimeDifferenceInDays(Utils.convertStringToDate("2018-10-20 16:16:16"), Utils.convertStringToDate("2018-10-10 16:16:16"));
        assertEquals(new Long(10), resultWithString);
        assertEquals(new Long(10), resultWithDate);

        resultWithString = Utils.getTimeDifferenceInDays(null, "2018-10-10 16:16:16");
        resultWithDate = Utils.getTimeDifferenceInDays(Utils.convertStringToDate(null), Utils.convertStringToDate("2018-10-10 16:16:16"));
        assertNull(resultWithString);
        assertNull(resultWithDate);

        resultWithString = Utils.getTimeDifferenceInDays("2018-10-20 16:16:16", null);
        resultWithDate = Utils.getTimeDifferenceInDays(Utils.convertStringToDate("2018-10-20 16:16:16"), Utils.convertStringToDate(null));
        assertNull(resultWithString);
        assertNull(resultWithDate);

        resultWithString = Utils.getTimeDifferenceInDays("", "2018-10-10 16:16:16");
        resultWithDate = Utils.getTimeDifferenceInDays(Utils.convertStringToDate(""), Utils.convertStringToDate("2018-10-10 16:16:16"));
        assertNull(resultWithString);
        assertNull(resultWithDate);

        resultWithString = Utils.getTimeDifferenceInDays("2018-10-20 16:16:16", "");
        resultWithDate = Utils.getTimeDifferenceInDays(Utils.convertStringToDate("2018-10-20 16:16:16"), Utils.convertStringToDate(""));
        assertNull(resultWithString);
        assertNull(resultWithDate);

        resultWithString = Utils.getTimeDifferenceInDays("", "");
        resultWithDate = Utils.getTimeDifferenceInDays(Utils.convertStringToDate(""), Utils.convertStringToDate(""));
        assertNull(resultWithString);
        assertNull(resultWithDate);
    }
    ...
    void convertDateToStringTest() {
        String result = Utils.convertDateToString(Utils.convertStringToDate("2018-10-10 16:16:16"));
        assertEquals("2018-10-10 04:16:16", result);
    }
    ...
    void convertStringToDateTest() {
        Date expected = null;
        try {
            expected = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").parse("2018-10-10 16:16:16");
        } catch (ParseException e) {
            e.printStackTrace();
        }
        Date result = Utils.convertStringToDate("2018-10-10 16:16:16");
        assertEquals(expected, result);
    }

    @Test
    void convertStringToDateExceptionTest() {
        Date result = Utils.convertStringToDate("dsakldksahdjkashdkhsadjkhasjkdhasjhdjkas");
        assertNull(result);
    }
